...
public class DateUtil {
	...
	public static SimpleDateFormat sdf66 = new SimpleDateFormat("yyyy年MM月dd日");
	...
	public static SimpleDateFormat sdfbb = new SimpleDateFormat("yyyy.MM.dd HH:mm:ss");
	public static SimpleDateFormat sdfbbb = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
	public static SimpleDateFormat sdfc = new SimpleDateFormat("yyyyMMdd HHmmss");
	...
	public static String getDateStringPattern3(String miliSecond) {
	    return sdf3.format(Long.valueOf(miliSecond));
    }
	...
	public static int daysBetween(Date date1, Date date2){
		long between_days = getDateDiffByDay(date1, date2);
...
//        return s1;
    ...
    public static String getRelativeTime(java.util.Date date, int iYear, int iMonth, int iDate, int iHour, int iMinite, int iSecond){
    	return getRelaitveTime(date, iYear, iMonth, iDate, iHour, iMinite, iSecond, HRRB_LONG_DATETIME_FORMAT);
    }
    
    public static String getRelaitveTime(java.util.Date date, int iYear, int iMonth, int iDate, int iHour, int iMinite, int iSecond, String format){
    	SimpleDateFormat sdf = new SimpleDateFormat(format);            //定义格式
        GregorianCalendar gc = new GregorianCalendar();            //
        gc.setTime(date);           //设置时间
        ...
        gc.add(Calendar.MINUTE, iMinite);
    ...
    /**
	 * 字符串到日期类型的转换，根据常用的日期格式猜测一种可能的转换，应该可以满足大部分的需要但
	 * 并不保证一定成功，支持的格式包括：
	 * <ul>4位年日期格式，以下格式都可以兼容同类型年在最后的形式(mdy)
	 * <li>yyyy/MM/dd
	 * <li>yyyy-MM-dd
	 * <li>yyyy.MM.dd
	 * <li>yyyy/M/d
	...
	public static Date parse2Date(String str){
		...
		for(int i=0;i<split.length;i++){
			...
			if(k<0){ //没有找到
				...
				format = 'M'+split[i]+'d'+split[i]+"yyyy";
				break;
			}else { //无效格式
				return null;
			}
		...
		if(format==null){ //未找到合适的贷分割的格式，用不是分割的方式
			if(str.substring(0,2).compareTo("12")>0){ //假定年份是以19xx/2xxx开头
				format = "yyyyMMdd";
			}else{
				format = "MMddyyyy";
			}
...
//    }
    ...
    public static String formatDate(Date date) {
        return formatDate(date, HRRB_SHORT_DATETIME_FORMAT);
    }
	...
	public static Date getDatePatternBB(String dateString) throws Exception {
		Date date = sdfbb.parse(dateString);
		return date;
	}
	
	public static Date getDatePatternBBB(String dateString) throws Exception{
		Date date = sdfbbb.parse(dateString);
		return date;
	}
   ...
   public void getTimeByDate(){
	...
	public static Date getDatePatternC(String dateString) {
        Date date = null;
		try {
