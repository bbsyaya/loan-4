...
public class LoanPosCreditApplyServiceImpl implements LoanPosCreditApplyService{
	...
	private Logger logger = LoggerFactory.getLogger(LoanPosCreditApplyServiceImpl.class);
...
//	}
	...
	public Map<String, Object> selectRepaymentPlayById(Map<String, Object> reqMap) {
		...
		logger.debug("in repaymentPlanQueryService....");
		Map<String, Object> resMap = Maps.newHashMap();
		Map<String, Object> payResMap = tPaymentApplyDao.queryPaymentApplyByIssueId(reqMap);
		logger.debug("payResMap= " + payResMap);
		List<Map<String, Object>> resList = tRepaymentPlanDao.getRepaymPlansByPayApplyId(reqMap);
		logger.debug("repaymentPlaysRes= " + resList);
		String count = tRepaymentPlanDao.countRepaymentPlans(reqMap);
		logger.debug("plansCount="+count);
		String recNum = String.valueOf(resList.size());
		String contNo = (String) payResMap.get("contNo");
		String custId = (String) payResMap.get("custId");
		resMap.put("issueid", reqMap.get("payApplyId"));
		resMap.put("contno", contNo);
		resMap.put("custid", custId);
		resMap.put("totalnum", count);
		resMap.put("retnum", recNum);
    ...
    public String getChannelByLoanId(String loanId) {
        return tCreditApplyDao.selectChannelByLoanId(loanId);
    }
    
    @Override
    public Long countDailyApply(Map<String,Object> reqMap){
        return tCreditApplyDao.countDailyApply(reqMap);   
    }
