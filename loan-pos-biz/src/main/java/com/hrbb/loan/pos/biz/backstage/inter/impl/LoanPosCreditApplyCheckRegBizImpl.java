...
public class LoanPosCreditApplyCheckRegBizImpl implements ILoanPosCreditApplyCheckRegBiz {
    ...
    public Map<String, Object> creditApplyCheck(String loanId) throws Exception {
        ...
        if (!ReviewNoteConstants.CHANNEL_CODE_58.equals(channel)) {
            if (creMap.get("region") != null) {
                //禁入地区
                String areaName = containsAreaByCode(creMap.get("region").toString(), channel);
    ...
    // guoyu 20150714 add
    // 加入 北京和南平
    //针对渠道channel，开放特定地区可以进件
    private String containsAreaByCode(String addr, String channel) {
        if (addr.startsWith("15")) {
        	if(BusinessDictionaryConstants.BIZ_CHANNEL_UM.equals(channel)){ //银商渠道channel=UM，开放内蒙地区可以进件
        		 return null;
        	}
            return "内蒙古";
