...
public class PaymentApplyCheckBizImpl implements IPaymentApplyCheckBiz {
    ...
    /** 
     * @see com.hrbb.loan.pos.biz.backstage.inter.IPaymentApplyCheckBiz#paymentApplyCheck(java.lang.String)
     */
    @Override
    public Map<String, Object> paymentApplyCheck(String payApplyId) {
        ...
        try {
            ...
            //4. 利随本清不超过3个月 ；等额本息不超过12个月
            String payApplyTerm = paymentApply.getPayApplyTerm();
            int iApplyTerm = Integer.parseInt(payApplyTerm);
            if(paymentApply.getReturnType()!=null && paymentApply.getReturnType().equals("10")){
            	//等额本息限定在12个月
                if (iApplyTerm > 12) {
                    return respMap("999", "用款期限超出12个月");
                }
            }else{
            	//用款期限限定在1~3个月
                if (iApplyTerm > 3) {
    ...
    /** 
     * @see com.hrbb.loan.pos.biz.backstage.inter.IPaymentApplyCheckBiz#checkCreditReport(java.lang.String)
     */
    @Override
