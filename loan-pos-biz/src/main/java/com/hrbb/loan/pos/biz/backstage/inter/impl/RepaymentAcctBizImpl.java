...
public class RepaymentAcctBizImpl implements IRepaymentAcctBiz {
    ...
    private static final Logger      logger = LoggerFactory.getLogger(RepaymentAcctBizImpl.class);
    ...
    public Boolean repayment(String paybackApplyId) {
        ...
        if (StringUtil.isNotEmpty(paybackApplyId)) {
            ...
            if (paybackApplyInfo != null) {
                TReceiptInfo receiptInfo = receiptManageBiz.selectReceiptByPrimary(paybackApplyInfo
                    .getReceiptId());
                if (receiptInfo != null) {
                    ...
                    if (StringUtil.isNotEmpty(loanAcNo)) {
                        try {
                            ...
                            if (StringUtil.isNotEmpty(listId)) {
                                ...
                                if (res != null
                                    && AcctRetConstants.SUCCESS.getValue()
                                        .equals(res.getRespCode())) {
                                    return true;
                                }
                            }
                        } catch (Exception e) {
     ...
     * @param listId
     */
    private MadeLoanExecTurnOverRes processTurnOver(String loanAcNo, String listId)
                                                                                   throws Exception {
     ...
     * @param receiptInfo
     */
    private String addReturnOverReq(TPaybackApplyInfo paybackApplyInfo, String loanAcNo)
                                                                                        throws Exception {
    ...
    public void updateRepaymentPlan(String receiptId) {
        try {
            if (StringUtil.isNotEmpty(receiptId)) {
                ...
                RepayCalmQueryReq req = new RepayCalmQueryReq();
                req.setLoanAcNo(loanAcNo);
                logger.info("查询分户账loanAcNo=" + loanAcNo + "的还款计划，调用核算doRepayCalm接口开始,req=[{}]", req);
                RepayCalmQueryRes res = madeLoanProcessQuerySimpleFacade.doRepayCalm(req);
                logger.info("查询分户账loanAcNo=" + loanAcNo + "的还款计划，调用核算doRepayCalm接口结束,res=[{}]", res);
                if (res != null) {
                    List<RepayClamQuerySubBean> list = res.getRepayList();
                    if (list != null && list.size() > 0) {
                        //删除还款计划
                        ...
                        for (RepayClamQuerySubBean plan : list) {
                            ...
                            rp.setSchedprincipal(plan.getsCapi() == null ? BigDecimal.ZERO
                                : new BigDecimal(plan.getsCapi()));//应还本金
                            rp.setSchedinterest(plan.getsInte() == null ? BigDecimal.ZERO
                                : new BigDecimal(plan.getsInte()));//应还利息
                            rp.setUnpaidinterest(plan.getsFine() == null ? BigDecimal.ZERO
                                : new BigDecimal(plan.getsFine()));//应还罚息
                            rp.setSchedtotalamt(rp.getSchedprincipal().add(rp.getSchedinterest())
                                .add(rp.getUnpaidinterest()));
