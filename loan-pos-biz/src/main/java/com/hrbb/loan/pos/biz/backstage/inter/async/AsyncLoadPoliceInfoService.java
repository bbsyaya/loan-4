...
public class AsyncLoadPoliceInfoService implements IAsyncLoadPoliceInfoService{
    ...
    @Autowired
    private PoliceInfoService policeService;
    @Autowired
    ...
    public TPoliceInfo loadPoliceInfoInTime(String loanId){
        return queryPoliceInfo(loanId, true);
    }
     ...
     */
    private TPoliceInfo queryPoliceInfo(String loanId, boolean isDirect){
        ...
        if(!isDirect)
            if(policeAndAICConnectBiz.hasPoliceInfo(reqMap.get("idNo"),reqMap.get("custName"))) 
                return policeService.queryPoliceInfoByLoadId(loanId);
