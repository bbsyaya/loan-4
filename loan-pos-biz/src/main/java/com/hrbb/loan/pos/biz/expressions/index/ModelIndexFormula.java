...
public class ModelIndexFormula {
	...
	public static TRiskCheckModelIndex initModelIndexBean(
		...
		if (StringUtil.isNotEmpty(cr.getCR029())) {
			try {
				cr029 = DateUtil.getDatePatternB(cr.getCR029());
			} catch (Exception e) {
		...
		if (StringUtil.isNotEmpty(cr.getCR002())) {
			try {
				cr002 = DateUtil.getDatePatternBB(cr.getCR002());
			} catch (Exception e) {
				
				logger.debug("cr002=" + cr.getCR002());
				try{
					cr002 = DateUtil.getDatePatternB(cr.getCR002());
				}catch(Exception e1){
					logger.error("再换种格式");
					try{
						cr002 = DateUtil.getDatePatternBBB(cr.getCR002());
					}catch(Exception e2){
						logger.error("换第三种格式");
						try{
		...
		if (cr.getCR057().compareTo(BigDecimal.ZERO) == 0) {
			mb.setModelIndex14(BigDecimal.ZERO);
		} else {
		...
		mb.setModelIndex16(MathUtils.div(cr.getCR060(), new BigDecimal(10000))
				.add(cr.getCR066()));
		...
		mb.setModelIndex53(cr.getCR066().add(cr.getCR110()));
		return mb;
	}
