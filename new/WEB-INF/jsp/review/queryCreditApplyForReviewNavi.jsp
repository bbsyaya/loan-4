	...
	<script type="text/javascript">
		...
		function queryCreditApplyForClaim(){
			...
			$('#tClaim').datagrid({url:queryUrl,
				...
				loanId: $('#searchLoanIdForClaim').val(),
		...
		function addReviewNote() {
			...
			$.post(reqUrl, 
				{loanid: row.loanId,
				 teltype: $('#rn_teltype').datebox("getValue"),
				 tel: $('#rn_tel').val(),
...
<body>
	...
	<div id="tb" style="padding:5px;height:auto">
		...
		<% if("1".equals(opflag)) {%>
		    ...
		    <% //if(privileges!=null && (privileges.indexOf("ROLE_APPR_LV3;")>=0)) {	//高级复审人员才能操作回池 %>
		    ...
		    <% if(privileges!=null && (privileges.indexOf("ROLE_INFO;")>=0 ||
		    		privileges.indexOf("ROLE_INFO_APPR;")>=0 || 
		    		privileges.indexOf("ROLE_APPR_LV1;")>=0
		    		)) {%>
			    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-tip" onclick="queryCreditReport(0)">获取征信报告</a>&nbsp;&nbsp;
		     <% } %>
		    
		    <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-ok" onclick="executeApproval()">执行审批</a>
		<% } else {%>
