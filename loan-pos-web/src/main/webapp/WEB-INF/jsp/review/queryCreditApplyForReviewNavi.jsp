...
<style type="text/css">
	...
	<script type="text/javascript">
        ...
        function switchFunctions(row){
        	...
        	var inChannelKindName = row["inChannelKindName"];
        	if(typeof(applyStutus)!="undefined" && applyStutus!=""){
        		if(applyStutus=="10"){
        			if($("#funcAddInfo").length>0){ $("#funcAddInfo").linkbutton("disable"); }
        			...
        			if(applyStutus == "21"){
        				if(typeof(inChannelKindName) != "undefined" && inChannelKindName == "自建"){
        					if($("#funcAddInfo").length>0){ $("#funcAddInfo").linkbutton("enable"); }
        				}else{
        					if($("#funcAddInfo").length>0){ $("#funcAddInfo").linkbutton("disable"); }
        				}
        			}else{
		...
		function queryCreditApplyForClaim(){
			...
			$('#tClaim').datagrid({url:queryUrl,
				...
				channel: $('#searchChannelForClaim').combobox('getValue'),
				occurType: $('#searchOccurTypeForClaim').combobox('getValue'),
				opflag: 0
				},
		...
		function queryCreditApply(){
			...
			$('#t').datagrid({url:queryUrl,
				...
				beginDateFrom: $('#searchBeginDateFrom').datebox("getValue"),
				beginDateTo: $('#searchBeginDateTo').datebox("getValue"),
		...
		//申请退回
		function backApply(){
			var backReason = "退回录入,补充材料";
		    var reqUrl = "<%=request.getContextPath()%>/creditApplyForReview/backReason.do";
		    var row = $('#t').datagrid('getSelected');
			$.post(reqUrl, 
				{loanid: row.loanId,
				 applyStatus: row.applyStatus,
		...
		function addInfoSubmit() {
			...
			var aie = $("input[name='apprInfoExtC']:checked").map(function () {
			 ...
			 var obj=document.getElementsByName('apprInfoExtC');
			 var s='';    
			  for(var i=0; i<obj.length; i++){    
			    if(obj[i].checked) s+=obj[i].value+'|';  //如果选中，将value添加到变量s中    
			  }   
			  var addinfo = s.substring(0,s.length-1);
		    $.messager.progress({
	 	        text: '数据处理中，请稍等......',
	 	    }); 
			$.post(reqUrl, 
		...
		//查询工商信息
		function getAicInfo(confirmFlag){
        	//初次查询
        	if (confirmFlag == 0){
	        	//检查是否只选择了一条记录
	        	if(!checkSelected()){
        	...
        	var reqUrl = "<%=request.getContextPath()%>/creditApply/getAicInfo.do";
		    var row = $('#t').datagrid('getSelected');
		    $.messager.progress({
	 	        text: '工商信息查询中，请等待......',
	 	    }); 
			$.post(reqUrl,
				...
				function(data){
					//查询时间小于20天
					if (data == '0'){
						$.messager.confirm("确认", "已查询过该商户的工商信息，是否重新查询？", function (r) {
					        if (r) {
					        	getAicInfo(1);
					        }
					    });
		...
		function interfaceProces(){
			...
			$.post(reqUrl,{
				...
				var bankCheckRespMapStr = "账户验证接口结果为："+ (typeof(data.bankCheckRespMap) == 'undefined' ? '失败' : data.bankCheckRespMap.respMsg);
			 	var airCheckRespMapStr = "工商接口查询结果为："+ (typeof(data.airCheckRespMap) == 'undefined' ? '失败' : data.airCheckRespMap.respMsg);
				var imgCheckRespMapStr = "影像件查询结果："+ (typeof(data.imgCheckRespMap) == 'undefined' ? '失败' : data.imgCheckRespMap.respMsg);
		...
		function finishedAddInfo(){
			var reqUrl = '<%=request.getContextPath()%>/creditApply/finishAddInfo.do';
			if (!checkSelected()){
				return;
			}
			var row = $('#t').datagrid("getSelected");
			var loanId = row.loanId;
			$.post(reqUrl,{loanId:loanId},
					function(data){
					alert(data);	
					$('#t').datagrid('reload');
			},'text')
	...
	<div id="tb" style="padding:5px;height:auto">
		...
		<div id="tb" style="padding:3px">
			<table>
				<tr>
					...
					<td>业务渠道:</td>
					<td>
					    <select id="searchChannel" name="searchChannel" class="easyui-combobox" data-options="width:100,panelHeight:80" editable=false>
				...
				<tr>
					...
					<td>申请备注</td>
					<td><input id="searchRemark" name="searchRemark" style="line-height:20px;border:1px solid #ccc"/></td>
				</tr>
				<tr>
					<td>申请日期:</td>
					<td><input type="text" id="searchBeginDateFrom" name="searchBeginDateFrom" class="easyui-datebox" size="10"/>
					              ～
					    <input type="text" id="searchBeginDateTo" name="searchBeginDateTo" class="easyui-datebox" size="10"/>
					    </td>
				</tr>
			</table>
			<br/>
		...
		<% if("1".equals(opflag)) {%>
		    ...
		    <%if(access.hasAnyPrivilege("ROLE_INFO_APPR")){ %>
		    	<a id="funcBackApply" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-back" onclick="backApply()">退回申请</a>&nbsp;&nbsp;<!-- 先回池，再该状态 -->
		    <% }%>
		    ...
		    <% } %>
		    <%if(access.hasAnyPrivilege("ROLE_INFO_APPR")){ %>
			    <a id="funaic" href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-tip" onclick="getAicInfo(0)">获取工商信息</a>&nbsp;&nbsp;
		    <!-- 用于10状态的专用func -->
		    ...
		    <a id="funcAddInfo" href="javascript:void(0)" class="easyui-linkbutton" iconClas="icon-edit" onclick="finishedAddInfo()" disabled>线下补件</a>&nbsp;&nbsp;
	...
	<table id="t" width=100% style="height:720px"	toolbar="#tb" onClickRow="clickRow" rownumbers="true" pagination="true">
		<thead>
			<tr>
				...
				<th field="recOrg" width="80px">展业机构</th>
				<th field="remark" width="150px">申请备注</th>
			</tr>
		</thead>
	</table>
