...
<html>
  <head>
	...
	function openReactivate(){
		  var reqUrl = "<%=request.getContextPath()%>/paybackApply/reActivatePaybackApply.do";
		  var rows = $('#tt').datagrid('getSelections');
			var length = rows.length;
			if (length == 0){
			    alert("请选择一条记录！");
	...
	<div id="tb" style="padding:5px;height:auto"> 
		...
		<div id="tb">   
		    ...
		    <c:if test="${paybackStatus!='99' and paybackStatus=='00'}">
		     ...
		     <c:if test="${paybackStatus=='50'}">
			    <a href="javascript:void(0)" id="02" class="easyui-linkbutton" iconCls="icon-tip" onclick="openReactivate()">重新激活</a>&nbsp;&nbsp;
			 </c:if>
		  </div> 
	</div>  
