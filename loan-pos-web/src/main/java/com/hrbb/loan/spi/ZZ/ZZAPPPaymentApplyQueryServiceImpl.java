...
public class ZZAPPPaymentApplyQueryServiceImpl extends ZZAPPFoundationServiceImpl {
    ...
    private final ImmutableSet<String> payOffFlagSet = new ImmutableSet.Builder<String>().add("91").add("92").add("93").add("94").add("90").build();
            ...
            for (Map.Entry<String, Object> en : tmpMap.entrySet()) {
                ...
                if ("liststat".equals(key)) {
                	tmpMap.put(key, zzAppPaymentStatusMap.get(tmpMap.get(key)));
                	 ...
                	 logger.debug("查询出的listStatStringMap=" + listStatStringMap);
     				if ("0".equals(listStatStringMap)) {
     					logger.debug("---说明通过");
     					logger.debug("查询出的loanExecuteStatus=" + tmpMap.get("loanExecuteStatus"));
