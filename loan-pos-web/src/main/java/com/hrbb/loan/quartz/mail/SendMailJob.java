	...
	protected boolean processJob() throws Exception {
		...
		if(records!=null && records.size()>0){
			for(TMailNotification record:records){
				...
				if(record.getMailTo()!=null && record.getMailTo().trim().length()>0){
                ...
                if(m<5){
                    try{
                        ...
                        int a=m+1;
                        logger.debug("消息[id="+record.getId()+"]第"+a+"次发送失败!");
                        Map<String,Object> map = Maps.newHashMap();
                        map.put("id",record.getId());
                        map.put("sendCount", a);
                        int flag = tMailNotificationService.updateCount(map);
                        if(flag==1){
