...
<head>
	...
	function openRiskchannelConfigListWindow(){
		...
		var reqUrl = "<%=request.getContextPath()%>/riskChannelConfig/riskChannelConfigDetail.do?id="+id;
	...
	function addRiskchannelConfigWindow(){
		var flag = validateForm("riskChannelDiv");
		if (!flag){
			$.messager.alert("操作提示","还有未填写的必输项或输入不符合要求","error");
			return;
		}
		var reqUrl = '<%=request.getContextPath()%>/riskChannelConfig/addRiskChannelConfig.do';
	    var channel = $('#riskChannel').combobox("getValue");
	    var risk_type = $('#risk_type_select').combobox("getValue");
		var deleted_flag = $('#deleted_flag_select').combobox("getValue");
		var valid_date = $('.valid_date').datebox("getValue");
		var invalid_date = $('.invalid_date').datebox("getValue");
		if(typeof(channel) == 'undefined' || $.trim(channel) == ''){
			alert('渠道不能为空');
			return;
		}
		$.post(reqUrl,{
	...
	<legend class='dialog-label'>查询条件</legend>
		<table width="800" border="0" cellspacing="1" cellpadding="0">
		  <tr>
		    ...
		    <td width="180">
		    	<select id="channel" class="easyui-combobox" style="width:180px;">
		    		<option value="">--请选择--</option>
		    		<c:forEach items="${channelDictionary}" var="obj">
