	...
	<script type="text/javascript">
       ...
       function bbb(){
        var rows = $('#tt').datagrid('getSelections');
            ...
            var row = $('#tt').datagrid('getSelected');
            var reqUrl = "<%=request.getContextPath()%>/contractManagement/queryDate2.do";
            var approveId = row.approveId;
			$.post(reqUrl, {approveId:approveId}, function(data){
				if(data==2){
       ...
       function ccc(){
            var reqUrl = "<%=request.getContextPath()%>/contractManagement/updateContractManagement5.do";
 			var row = $('#tt').datagrid('getSelected');
	...
	<table id="tt" class="easyui-datagrid" style="width:1500px;height:800px"
		...
		<thead>
			<tr>
				...
				<th field="applyAmt" width="120" hidden="true" hidden="true">申请额度</th>
			    <th field="applyMoneyKind" width="120" hidden="true" hidden="true">申请币种</th>
			    <th field="termUnit" width="120" hidden="true" hidden="true">期限单位</th>
			    <th field="applyTerm" width="120" hidden="true" hidden="true">申请期限</th>
			    <th field="applyCommitDateStr" width="120" hidden="true" hidden="true">申请提交日期</th>  
			    <th field="acceptAccount" width="120">收款账号</th>
			    <th field="accountOpenBank" width="120">账户开户行</th>
				<th field="loanId" width="120" hidden="true" hidden="true">申请编号</th>
				<th field="custId" width="120">客户编号</th>
				<th field="approveAmount" width="120">批复额度</th>
				<th field="approveMoneyKind" width="120" hidden="true">批复币种</th>
				...
				<th field="approveStatus" width="120" hidden="true">批复状态</th>
