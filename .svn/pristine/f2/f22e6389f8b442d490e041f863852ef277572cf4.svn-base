...
<title>业务申请</title>
	...
	function modifyCreditApply(){
		...
		var cols = {//channel: channel,custName: custName,
				//paperKind: paperKind,
				applyAmt: applyAmt,
				custName: custName,
    ...
    //获取审批信息
    function getInforAuditList(loanId){
		...
		$.post(reqUrl,
			...
			function(obj){
				var htmlStr = "";
				if(obj.length > 0){
					var htmlStr_tb_begin = "<br/><table border=1 style='border-collapse:collapse;'>";
					var htmlStr_tr1_begin = "<tr><td>阶段名称</td><td colspan='3'>";
					var htmlStr_tr2_begin1 = "<tr><td width='100px'>提交时间</td><td width='100px'>";
					var htmlStr_tr2_begin2 = "</td><td width='100px'>认领时间</td><td width='100px'>";
					var htmlStr_tr3_begin = "<tr><td>审批意见</td><td colspan='3'>";
					var htmlStr_tr4_begin = "<tr><td>意见详情（内部）</td><td colspan='3'>";
					var htmlStr_tr_end = "</td></tr>";
					var htmlStr_tb_end = "</table><br/>";
					var htmlStr_div_end = "</div>";
					for(var i=0; i<obj.length; i++){
						htmlStr = htmlStr.concat("<table style='background-color:gray'><tr><td id='alTD"+i+"' onclick='showInforAuditList("+i+")' style='cursor: pointer;' width='10px'>-</td><td id='ialTD"+i+"' onclick='showInforAuditList("+i+")' style='cursor: pointer;'>"+obj[i].apprStateTitle+"</td></tr></table>");
						htmlStr = htmlStr.concat("<div id='auditRow"+i+"'>");
						htmlStr = htmlStr.concat(htmlStr_tb_begin).concat(htmlStr_tr1_begin).concat(obj[i].apprStateName).concat(htmlStr_tr_end);
						htmlStr = htmlStr.concat(htmlStr_tr2_begin1).concat(obj[i].appEndTimeStr).concat(htmlStr_tr2_begin2).concat(obj[i].appBeginTimeStr).concat(htmlStr_tr_end);
						htmlStr = htmlStr.concat(htmlStr_tr3_begin);
						if (typeof(obj[i].apprResult) != "undefined"){
							htmlStr = htmlStr.concat(obj[i].apprResult).concat(htmlStr_tr_end);
						}
						htmlStr = htmlStr.concat(htmlStr_tr4_begin);
						if (typeof(obj[i].apprInfo) != "undefined"){
							htmlStr = htmlStr.concat(obj[i].apprInfo).concat(htmlStr_tr_end);
						}
						htmlStr = htmlStr.concat(htmlStr_tb_end).concat(htmlStr_div_end);
					}
				}
				$('#inforAuditList').html(htmlStr);
				$('#inforAuditList').show();
			},'json'
		);
	}
	...
	// 提交资料审批
	function submitCreditApplyApproval(){
		var reqUrl = '<%=request.getContextPath()%>/creditApply/submitCreditApplyApproval.do';
