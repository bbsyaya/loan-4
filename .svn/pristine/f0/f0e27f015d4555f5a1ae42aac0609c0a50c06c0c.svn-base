	...
	$.fn.zyImage = function(options){
		...
		return this.each(function(){
			...
			this.createThumImageHtml = function(){
				...
				var html = '';
				html += '<div class="thum_container mc-thumb-list mc-dir-v" align=center style="background-color:'+bgColor+';height:'+opts.thumImageHeight+'px;">';
			...
			this.bindMainImageEvent = function(){
				...
				$(self).find(".mc-nav-prev").bind("click", function(e){
					...
					if(selectedIndex == -1){
						selectedIndex = opts.imgList.length-1;
					}
				...
				$(self).find(".mc-nav-next").bind("click", function(e){
					...
					selectedIndex = selectedIndex+1;
					if(selectedIndex === opts.imgList.length){  // 当前索引是不是图片的最后一个
						selectedIndex = 0;
					}
			...
			this.bindThumImageEvent = function(){
				...
				$(self).find(".thum-nav-prev").bind("click", function(e){
					...
					if(selectedIndex === -1){  // 当前索引是不是图片的第一张
						selectedIndex = opts.imgList.length-1
					}			
			...
			this.moveThumbById = function(direction){
				if(direction=="next"){
					if (selectedIndex == 0) {
						thumbsLeft = opts.thumImageWidth;
						// ie8兼容
						if(navigator.appName == "Microsoft Internet Explorer" && navigator.appVersion.match(/8./i)=="8."){
							$(self).find(".mc-thumbs-cont").stop().animate({left:thumbsLeft+"px"},300);
						}else{
						...
						if(opts.mainImageWidth <= opts.thumImageWidth*(selectedIndex+1)){
							...
							if(opts.thumImageWidth*(selectedIndex+1)+thumbsLeft>=opts.mainImageWidth){  // 不是，移动
								...
								thumbsLeft = thumbsLeft - opts.thumImageWidth*7;
								// ie8兼容
								if(navigator.appName == "Microsoft Internet Explorer" && navigator.appVersion.match(/8./i)=="8."){
									$(self).find(".mc-thumbs-cont").stop().animate({left:thumbsLeft+"px"},300);
								}else{
					...
					if (selectedIndex == opts.imgList.length-1) {
						thumbsLeft = (1-opts.imgList.length)*opts.thumImageWidth;
