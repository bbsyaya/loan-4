...
//					alert(recPerson+" / "+recOrg+" / "+recManager);
    ...
    */
	...
	//打开复审调件窗口
	function openAdjustApproval(){
		if(!checkSelected()){
			return;
		}
		var row = $("#tt").datagrid("getSelected");
		var applyStatus = row.applyStatus;
		if("31" != applyStatus && "33" != applyStatus && "34" != applyStatus && "35" != applyStatus){
			alert("不是复审中的申请，不能调件!");
		}
		var loanId = row.loanId;
		$("#adjustApprovalWindow").openWin({
			title: '复审调件',
			width: 650,
			height: 300,
			modal: true,
			...
			closed: true,
			resizable: false,
			href: "<%=request.getContextPath() %>/sign/openAdjustApproval.do?loanId=" + loanId
			
		});
	}
	
	//复审调件
	function adjustApproval(){
		var loanId = $("#adjustLoanId").val();
		var applyStatus = $("#approvalLevel").combobox('getValue');
		var applyStatusName = $("#approvalLevel").combobox('getText');
		var userName = $("#approvalPerson").combobox('getValue');
		var loginName = $("#approvalPerson").combobox('getText');
		if(checkApprovalInfo(applyStatus, userName)){
			if(confirm("确定要调件吗？")){
				var reqUrl = "<%=request.getContextPath()%>/sign/adjustApproval.do?loanId=" + loanId;
				$.post(reqUrl,
	...
	//关闭复审调件窗口
	function closeAdjustApprovalWindow(){
		$("#adjustApprovalWindow").window('close');
	}

	//级联修改复审人选项
	function selectApprovalPerson(applyStatus, userBox){
		if(applyStatus != ""){ 
			var user = userBox.combobox({
				url: "<%=request.getContextPath()%>/user/queryUsersByApplyStatus.do?applyStatus=" + applyStatus,
				valueField:'userName',
				textField:'loginName',
				onLoadSuccess:function(data){
					userBox.combobox('clear');
					var combobox = userBox.combobox('getData');
					userBox.combobox('setText', combobox[0].loginName).combobox('setValue', combobox[0].userName);
				}
			});
		}else{
			userBox.combobox('loadData',"[]");
			userBox.combobox('clear');
		}
	...
	//复审调件校验
	function checkApprovalInfo(applyStatus, userName){
		if(typeof(applyStatus) == 'undefined' || applyStatus.length ==0){
			alert("请选择复审阶段!");
			return false;
		}
...
<body>
	<table id="tt" style="height:600px"
		...
		<thead>
			<tr>
			    ...
			    <th field="applyStatus" hidden="true"></th>
	...
	<div id="adjustApprovalWindow"></div><!-- 复审调件 -->
