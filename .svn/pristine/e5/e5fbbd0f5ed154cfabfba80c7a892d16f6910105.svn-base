...
//					logger.debug("账户验真中 cardName:" + tCreditApply.getCustName());
//					bankCheckMap.put("cardName", tCreditApply.getCustName());
//					logger.debug("账户验真中cardNo:" + tCreditApply.getBankAccno());
//					bankCheckMap.put("cardNo", tCreditApply.getBankAccno());
//					
//					bankCheckMap.put("orderNo", IdUtil.getBankCheckSeqNo());
//					logger.debug("账户验真中certNo:" + tCreditApply.getPaperId());
...
//					bankCheckMap.put("channel", tCreditApply.getChannel());
					...
					TCustomer customer = loanPosCustomerService.getCustumerInfoById(tCreditApply.getPaperId());
					if(customer != null){
						Map<String, String> policeMap = Maps.newHashMap();
						policeMap.put("custName", tCreditApply.getCustName());
						policeMap.put("idNo", tCreditApply.getPosCustName());
						runnable.setPoliceMap(policeMap);	
					}
					//runnable.setBankCheckMap(bankCheckMap);
					// 外部验真
					Map<String, Object> updateMap = Maps.newHashMap();
					updateMap.put("status", "03");
					updateMap.put("bankAccno", tCreditApply.getBankAccno());
