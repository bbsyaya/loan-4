...
<script type="text/javascript">
	...
	function openReviewNoteWindow(rowData){
		...
		if (applyStatusCode == '31' || applyStatusCode == '32' || applyStatusCode == '33' || applyStatusCode == '34'|| applyStatusCode == '40' 
				|| applyStatusCode == '41'|| applyStatusCode == '35'|| applyStatusCode == '90' || applyStatusCode == '92'){
			var url = "<%=request.getContextPath()%>/reviewNote/openReviewEditView.do?loanId=${loanId}&direct2Path=review/reviewNote&flag=2&reviewId="+rowData.reviewid+"&applyStatus="+applyStatusCode;
			$('#reviewNoteWindow').openWin({
				title: '更新照会记录',
				modal : false,
				width: 550,
