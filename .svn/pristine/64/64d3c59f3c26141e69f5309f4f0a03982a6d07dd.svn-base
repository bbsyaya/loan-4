...
//            contractInfo.setAccountOpenBank(accountOpenBank);
            ...
            resMap.put("accountOpenBank", accountOpenBank);
            logger.debug("getContractInfo success contNo:[]", contNo);
	...
	public void addContractStatusChangeMessage(String contNo, TContractManagement contract, String contStatus, String adjustReason) {
        try {
           ...
           if(tCreditApply != null){
            ...
            if(contract.getChannel()!=null && contract.getChannel().equals("UM")){
                ...
                messageInfo.setAdjustDate(DateUtil.getDateToString12(new Date()));
            }
...
//            }
            ...
            record.setInChannelKind(contract.getInChannelKind());
            //入库
	...
	public TContractManagement getContractByPayApplyId(String payApplyId) {
		return loanPosContractManagementService.getContractInfoByPayapplyId(payApplyId);
	}
    ...
    public Map<String, Object> transferToPdfArgee(Map<String, Object> contractMap){
        ...
        map.put("contNo", contractMap.get("contNo"));
        map.put("custName", contractMap.get("custName"));
        map.put("paperId", contractMap.get("paperId"));
        map.put("apptcapiZh", MoneyUtils.digitUppercase(((BigDecimal) contractMap.get("apprTotalAmt")).doubleValue()));
        map.put("apptcapi", MoneyUtils.format(((BigDecimal) contractMap.get("apprTotalAmt"))));
        Date beginDate = (Date) contractMap.get("beginDate");
        if(null != beginDate){
            String dateStr = DateUtil.formatDate(beginDate, DateUtil.HRRB_SHORT_DATETIME_FORMAT_BRIEF);
            String beginYear = dateStr.substring(0, 4);
            String beginMonth = dateStr.substring(4, 6);
            String beginDay = dateStr.substring(6, 8);
            map.put("beginYear", beginYear);
            map.put("beginMonth", beginMonth);
            map.put("beginDay", beginDay);
        }
        Date endDate = (Date) contractMap.get("endDate");
        if(null != endDate){
            String dateStr = DateUtil.formatDate(endDate, DateUtil.HRRB_SHORT_DATETIME_FORMAT_BRIEF);
            String endYear = dateStr.substring(0, 4);
            String endMonth = dateStr.substring(4, 6);
            String endDay = dateStr.substring(6, 8);
            map.put("endYear", endYear);
            map.put("endMonth", endMonth);
            map.put("endDay", endDay);
        }
        map.put("interate", ((BigDecimal)contractMap.get("approveInterest")).toString());
        String loanBankBranchName = (String) contractMap.get("accountBranchBank");
        if(null != loanBankBranchName){
            loanBankBranchName = loanBankBranchName.replaceAll("分行", "");
        }
        map.put("loanBankBranchName",loanBankBranchName);
        String loanBankSubbName = (String) contractMap.get("accountSubBranchBank");
        if(null != loanBankSubbName){
            loanBankSubbName = loanBankSubbName.replaceAll("支行", "");
        }
        map.put("loanBankSubbName", loanBankSubbName);
        map.put("loanBankAcno", contractMap.get("acceptAccount"));
        map.put("loanBankacName", contractMap.get("custName"));
        String repayBankBranchName = (String) contractMap.get("accountBranchBank");
        if(null != repayBankBranchName){
            repayBankBranchName = repayBankBranchName.replaceAll("分行", "");
        }
        map.put("repayBankBranchName", repayBankBranchName);
        String repayBankSubbName = (String) contractMap.get("accountSubBranchBank");
        if(null != repayBankSubbName){
            repayBankSubbName = repayBankSubbName.replaceAll("支行", "");
        }
        map.put("repayBankSubbName", repayBankSubbName);
        map.put("repayBankAccno", contractMap.get("acceptAccount"));
        map.put("repayBankAccName", contractMap.get("custName"));
        return map;
    }
}
