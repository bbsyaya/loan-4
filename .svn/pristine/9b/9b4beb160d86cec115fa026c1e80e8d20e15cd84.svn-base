$(function(){
	$('[hrbb-show-detail]').on('click',showDetail);
	$('[hrbb-delete-one]').on('click',deleteOne);
	$('[hrbb-delete-all]').on('click',deleteAll);
	selectAll();//全选与子选
});
...
/**
 * 删除一个对象，并删除改对象的父tr标签。
 * hrbb-delete-one ： 无属性值
 * hrbb-delete-url :(可选)删除路径，如果包含href属性，则优先使用herf属性 
 */
...
/**
 * 批量删除。
 * hrbb-delete-all ：要删除的checkbox 类名称
 * hrbb-delete-url :(可选)删除路径，如果包含href属性，则优先使用herf属性 
 */
function deleteAll(e){
	...
	var path =	getDeleteUrl($(this));
	if(!path) return ;
	var $item = $("."+$(this).attr("hrbb-delete-all")+":checked");
	if($item.length == 0){
...
function getDeleteUrl($this){
	...
	path = (!path) ? $this.attr("hrbb-delete-url") : path;
	if(!path){
...
function selectAll(){
	//全选事件绑定
	$('[hrbb-select-all]').on('click',function(){
		var selectItem = $(this).attr('hrbb-select-all');
		$('.'+selectItem+':checkbox').prop("checked",this.checked);
	});
	//子选事件绑定
	$('[hrbb-select-all]').each(function(i){
		var selectItem = $(this).attr('hrbb-select-all');
		var $item = $('.'+selectItem+':checkbox');
		$item.on("click",function(){
	 		$('[hrbb-select-all="'+selectItem+'"]').prop('checked',$item.length==$item.filter(':checked').length);
		 });
	});
}
