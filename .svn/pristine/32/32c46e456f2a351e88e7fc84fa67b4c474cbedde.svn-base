...
public class PaymentApplyApprovalBizImpl implements IPaymentApplyApprovalBiz {
    ...
    @Autowired
    PaymentApplyBackpoolService paymentApplyBackpoolService;
        ...
        if(null != lists && lists.size()>0){
            for (Map<String, Object> m : lists) {
                if(null != m.get("status")){
                   String statusName = loanPosBusinessCodeBiz.getItemNameByNo("IssueApplyStatus", (String)m.get("status"));
...
//		        senMessage(paymentApply, "0", (String)reqAppr.get("approvalContent"));
		        ...
		        source.setAttribute(POSEvent.监听事件名称_消息推送, MessageCab.消息推送_放款结果通知);
...
//            }
    ...
    @Override
    public boolean submitTaskBackPool(String payApplyId,String backPoolReason,String userName) {
        // 删除待回池任务
        try {
            paymentApplyApprovalService.deleteByPayApplyId(payApplyId);
            TPaymentApplyBackpool record = new TPaymentApplyBackpool();
            record.setPayApplyId(payApplyId);
            record.setBackPoolReason(backPoolReason);
            record.setCreateTime(new Date());
            record.setOperator(userName);
            // 插入回池任务
            paymentApplyBackpoolService.savePaymentApplyBackpool(record);
            return true;
        } catch (Exception e) {
            logger.error("用款任务回池失败", e);
            return false;
        }
    }
