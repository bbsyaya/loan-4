...
<fieldset>
	<legend class='dialog-label'>查询条件</legend>
		<table width="800" border="0" cellspacing="1" cellpadding="0">
		  <tr>
		    ...
		    <td width="180">
		    	<select id="channel_rtype_code" class="easyui-combobox" style="width:180px;">
		    		<option value="">--请选择--</option>
  ...
  function addApprovingAmountRateUplimitConfigWindow(){
	var flag = validateForm("amtRateUpLimitDiv");
	if (!flag){
    ...
    var channel_rtype_code = $('#amt-channel-select').combobox("getValue");
    var region_rtype_code = $('#amt-region-code').combobox("getValue");
    var prodId = $('#amt-prodId').combobox("getValue");
    var repay_type = $('#amt-repay_type').combobox("getValue");
    var deleted_flag = $('#amt_deleted_flag').combobox("getValue");
...
//    clearInstitutionWin();
  ...
  function executeQeury(){
    var reqUrl = '<%=request.getContextPath()%>/approvingAmountRateUplimitConfig/queryApprovingAmountRateUplimitConfig.do';
      ...
      var channel_rtype_code = $('#channel_rtype_code').combobox("getValue");
      $('#tt').datagrid({
        url : reqUrl,
        queryParams : {
          prodId : prodId,
  ...
  function queryBlank(){
     ...
     $('#channel_rtype_code').combobox("setValue", "");
  }
