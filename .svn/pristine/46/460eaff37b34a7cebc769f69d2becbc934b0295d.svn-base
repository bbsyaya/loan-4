...
import com.hrbb.loan.web.domain.ControllerPath;
 ...
 * 基础控制器接口实现类
 */
public abstract class BaseControllerImpl<T extends Identifiable<K>, Q extends T, K> implements BaseController<T, Q, K> {
	...
	public Result deleteList(K[] ids) {
	...
	public Result deleteOne(@PathVariable("id") K id) {
	...
	public ModelAndView viewOne(@PathVariable("id") K id) {
	...
	public ModelAndView editView(@PathVariable("id") K id) {
		Object obj = getBaseService().queryById(id);
