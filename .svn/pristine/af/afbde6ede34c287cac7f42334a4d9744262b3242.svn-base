...
<script type="text/javascript">
        ...
        function checkSelected(){
        	var rows = $('#synFileSummaryT').datagrid('getSelections');
		...
		function uploadSynFile() {
		    ...
		    var row = $('#synFileSummaryT').datagrid('getSelected');
		    var fileName = row.fileName;
        	if (fileName.indexOf('_UM_') == -1){
        		alert("请选择银商报表！");
        		return;
        	}
			var reqUrl = '<%=request.getContextPath()%>/synFileSummary/uploadSynFile.do';
		    $.messager.progress({
	 	        text: '上传中，请等待......',
	 	    }); 
			$.post(reqUrl, 
