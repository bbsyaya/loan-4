						...
						<table><tr>
							...
							<select  id="detailResidentProv" name="detailResidentProv" disabled="disabled">
								<c:forEach items="${province}" var="obj">
									<option value="${obj.itemNo}" <c:if test="${obj.itemNo == customer.residentProv}">selected</c:if>>${obj.itemName}</option>
								</c:forEach>
							</select>
							&nbsp;
							<select id="detailResidentCity"	name="detailResidentCity" disabled="disabled">
								<c:forEach items="${province}" var="obj">
									<option value="${obj.itemNo}" <c:if test="${obj.itemNo == customer.residentCity}">selected</c:if>>${obj.itemName}</option>
								</c:forEach>
							</select>
							&nbsp;
							<select id="detailResidentDivision" name="detailResidentDivision" disabled="disabled">
								<c:forEach items="${province}" var="obj">
									<option value="${obj.itemNo}" <c:if test="${obj.itemNo == customer.residentDivision}">selected</c:if>>${obj.itemName}</option>
								</c:forEach>
							</select>
							&nbsp;(详细)
							<input type="text" id="detailResidentDetail"
								name="detailResidentDetail" readonly="readonly" value="${customer.residentDetail}"/>
							</td>
							</tr></table>
						 -->
						</td>
