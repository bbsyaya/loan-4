 ...
 * 用途：检查输入字符串是否为空或者全部都是空格 输入：str 返回： 如果全是空返回true,否则返回false
 */
function isFineLens(str, min, max) {
	if (str == "")
		return true;
	if (!isNull(str)){
 ...
 * 金额
 * 
 * 用途：检查输入字符串是否符合金额格式 格式定义为带小数的正数，整数14位，小数2位  输入： s：字符串 返回： 如果通过验证返回true,否则返回false
 * 
 */
function isMoneyInt14Dec2(s) {
	var regu = "^[0-9]{1,14}([\.][0-9]{1,3})?$";
	var re = new RegExp(regu);
	if (re.test(s)) {
		return true;
	} else {
 ...
 * 
 */
function isRateInt3Dec6(s) {
	var regu = "^[0-9]{1,3}([\.][0-9]{1,6})?$";
	var re = new RegExp(regu);
	if (re.test(s)) {
		return true;
	} else {
